-- Signal Protocol compliant database schema
...
CREATE TABLE users (
  ...
  identity_key_private TEXT NOT NULL, -- Encrypted with user password
...
CREATE POLICY "Allow public key lookup" ON users
  FOR SELECT TO anon
  USING (true); -- Public keys are discoverable for key agreement
...
CREATE POLICY "Allow prekey fetching" ON prekeys
  FOR SELECT TO anon
  USING (used = FALSE); -- Only unused prekeys are fetchable
...
-- Functions for Signal Protocol operations

-- Generate one-time prekeys
CREATE OR REPLACE FUNCTION generate_one_time_prekeys(
  p_user_id UUID,
...
DECLARE
  ...
  max_key_id INTEGER;
BEGIN
  -- Get the highest existing key_id for one-time prekeys
  SELECT COALESCE(MAX(key_id), 0) INTO max_key_id
  FROM prekeys 
  WHERE user_id = p_user_id AND key_type = 'one_time_prekey';
  
  -- Generate new one-time prekeys
  FOR i IN 1..p_count LOOP
    INSERT INTO prekeys (user_id, key_type, key_id, public_key)
    VALUES (
      p_user_id,
      'one_time_prekey',
      max_key_id + i,
      encode(gen_random_bytes(32), 'base64') -- Placeholder - should be actual X25519 public key
    );
  END LOOP;
...
-- Mark prekey as used (for one-time prekeys)
CREATE OR REPLACE FUNCTION use_prekey(
  p_user_id UUID,
  p_key_type TEXT,
  p_key_id INTEGER
)
